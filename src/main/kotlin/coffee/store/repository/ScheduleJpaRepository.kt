...
interface ScheduleJpaRepository : CrudRepository<Schedule, Long> {
    @Query(
            value = "select расписание.id, расписание.название as name, расписание.состояние as status, расписание.описание as description, avg(о.оценка) as avgRating from расписание LEFT OUTER join оценка_расписания ор on расписание.id = ор.id_расписания LEFT OUTER join оценка о on ор.id_оценки = о.id where расписание.id_клиента = ? group by расписание.id, расписание.название, расписание.состояние, расписание.описание",
    ...
    @Query(
            value = "select расписание.id, расписание.название as name, расписание.состояние as status, расписание.описание as description, avg(о.оценка) as avgRating from расписание LEFT OUTER join оценка_расписания ор on расписание.id = ор.id_расписания LEFT OUTER join оценка о on ор.id_оценки = о.id where состояние = 'PUBLISHED' group by расписание.id, расписание.название, расписание.состояние, расписание.описание",
    ...
    @Query(
            value = "select copySchedule(?, ?)",
            countQuery = "select 1",
            nativeQuery = true)
    fun copySchedule(scheduleId: Long, clientId: Long): Long
}